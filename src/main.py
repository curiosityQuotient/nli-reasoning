...
from src.data.dataset import get_dataset, get_nli_dataset, load_nli_data
...
def prepare_datasets(
    df_train: Any, df_test: Any, source: str = "kaggle"
    ...
    if TRAIN_FRACTION == 1.0:
        ...
        val_nli_dataset = nli_dataset[int(len(nli_dataset) * TRAIN_FRACTION) :].repeat(
            NUM_EPOCHS
        )
    ...
    gsm8k_dataset = get_dataset(TRAIN_DATA_DIR, "train", source).batch(
        TRAIN_MICRO_BATCH_SIZE
    )[:NUM_BATCHES]
    ...
    test_gsm8k_dataset = get_dataset(TEST_DATA_DIR, "test", source).batch(
        TRAIN_MICRO_BATCH_SIZE
    )[:NUM_TEST_BATCHES]
...
def setup_model(
    ...
    Returns:
        Tuple of (reference_model, lora_model, tokenizer, mesh, model_config)
    """
    ...
    if not (resaved_ckpt_path / "model_params").exists():
    ...
    ref_model, mesh, model_config = get_gemma_ref_model(
        str(resaved_ckpt_path / "model_params")
    )

    print("Applying LoRA to create policy model...")
    lora_model = get_lora_model(ref_model, mesh)

    print("Getting tokenizer...")
    tokenizer_path = Path(kaggle_ckpt_path) / "tokenizer.model"
    tokenizer = get_tokenizer(
        tokenizer_path=str(tokenizer_path) if tokenizer_path.exists() else None
    )

    return ref_model, lora_model, tokenizer, mesh, model_config


def make_sampler(model: Any, tokenizer: Any, model_config: Any) -> Any:
    """Create a Tunix sampler with a cache sized for our generation budget.

    Args:
        model: Transformer (typically the LoRA policy model)
        tokenizer: Gemma tokenizer
        model_config: Model config providing cache sizing parameters
    ...
    return sampler_lib.Sampler(
        ...
        cache_config=sampler_lib.CacheConfig(
            cache_size=MAX_PROMPT_LENGTH + TOTAL_GENERATION_STEPS + 1024,
...
def run_pre_training_evaluation(
    test_nli_dataset: Any, test_gsm8k_dataset: Any, sampler: Any
) -> Tuple[Any, Any]:
...
def main(training_config: Optional[Dict[str, Any]] = None) -> None:
    ...
    df_train, df_test = load_nli_data(nli_data_path, test_size=200)
    ...
    ref_model, lora_model, tokenizer, mesh, model_config = setup_model(checkpoint_dir)
    ...
    try:
        sampler = make_sampler(lora_model, tokenizer, model_config)
    ...
    try:
        trained_sampler = make_sampler(trained_model, tokenizer, model_config)
        ...
        if nli_baseline is not None and gsm8k_baseline is not None:
